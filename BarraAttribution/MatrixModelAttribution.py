...
class MatrixModelAttribution():
    ...
    # --------------------------------------------------------------------
    def __init__(self, pool, benchmark):
        # self.data_engine = MatrixLibData('ribun', 'supwin888')  # 数据工具
    ...
    # --------------------------------------------------------------------
    def get_quote(self,start_date, end_date, code):
        url = "https://matrix.lab.supwin.com:7555/api/StockQuotes"
        header = {'Content-Type':'application/json','Accept': 'application/json'}
        data = str({"Codes": code, "StartDate": start_date, "EndDate": end_date,"IsAdjustQuotes": "0"})
        response = requests.post(url,headers = header,data = data)
        return response.json()["Result"][0]['PercentChange']*0.01
    ...
    def get_SupwinData_Fast(self, date):
        ...
        Position = pd.DataFrame(
            self.data_engine_fast.get(index="cnfactor", doc_type="cnfactor", id="cnaall-" + D + "-close")['_source'][
                'items'])
        self.Bench_Return = self.get_quote(start_date=D, end_date=D, code=symbols_map[self.benchmark])
    ...
    def get_SupwinAttribute(self):
        ...
        for index, date in enumerate(Date):
            if (index+1)%((len(Date)+10-1)//10) == 0:
                ...
                # requests.get(url="http://?id="+self.id+"&p="+str((index+1)/len(Date)))
...
def main():
    MatrixModelAttribution(pool="中证全指", benchmark="沪深300").output1()
    # fl = open(u'./归因分析_建宏/temp12.js', 'w')
    # fl.write("output=")
    # output = MatrixModelAttribution(pool="中证全指", benchmark="中证500").output1()
    # fl.write(output)
    # fl.close()
