...
from lib.MatrixLibData_new import MatrixLibData as SupwinData
...
class MatrixModelCovariancePredictor(object):
    ...
    def get_daily_return(self, start_date=None):
        ...
        for date in tradate:
            factors_return = self.data_engine.get_factor(date=date, pool=self.pool,config=["factors_return"])
            special_return = self.data_engine.get_factor(date=date, pool=self.pool,config=["special_return"])
    ...
    def covariance_predict(self, start_date=None, FactorCovariance=True, SpecificRisk=True, output=False, frequency="M",
                           Period="Short"):
        """
        :param start_date:
        ...
        cov_pre_dat = self.data_engine.get_trade_date(start_date=start_date, end_date=self.date,
                                                      period=frequency)  # 协方差的起始
        ...
        return pd.DataFrame(pd.Series(factor_risk), columns=[self.date]), pd.DataFrame(specific_risk.values,
                                                                                       index=specific_risk.index,
                                                                                       columns=[self.date])

class factor_risk_and_special_risk():
    def run(self, param):
        config = ["Risk_Short_W"]
        ...
        factors_risk, special_risk = barra_engine.covariance_predict(start_date=date, frequency=params[2],
                                                                     Period=params[1])
        factors_risk.reset_index(inplace=True)
        factors_risk.columns = ["Key", "Value"]
        special_risk.reset_index(inplace=True)
        special_risk.columns = ["Key", "Value"]
        return {'factors_risk': factors_risk.to_json(orient='records'),
                'special_risk': special_risk.to_json(orient='records')}


if __name__ == "__main__":
    param = {'date': '20180409', 'pool': '中证800'}
    coskew = factor_risk_and_special_risk()
