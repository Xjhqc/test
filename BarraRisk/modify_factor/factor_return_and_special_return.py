...
class MatrixModelCovariancePredictor(object):
    def __init__(self, date="20171231", pool="中证800", date_len=1, output=False):
        ...
        self.data_engine = SupwinData(username="yingying", password="matrix")
    ...
    def get_daily_return(self, start_date=None):
        ...
        if start_date:
            ...
            last_date_start = self.data_engine.get_trade_date(end_date=start_date)[-2]
            # print (u"将获取"+start_date+u"到"+self.date+u"的因子收益及特质收益")
        else:
            date_start = self.data_engine.trade_date_offset(date=self.date, offset=-int(self.date_len * 250))
            last_date_start = self.data_engine.trade_date_offset(date=self.date, offset=-int(self.date_len * 250) - 1)
        tradate = self.data_engine.get_trade_date(start_date=date_start, end_date=self.date)  # 获取期间所有交易日
        last_tradate = self.data_engine.get_trade_date(start_date=last_date_start, end_date=self.date)
        ...
        for index, date in enumerate(tradate):
            ...
            print(last_date,self.all_factor,self.pool)
            ...
            # 获取因变量：下日股票收益率
            data_y = self.data_engine.get_quote(end_date=str(date), start_date=str(date), code=X.Code.tolist())
...
if __name__ == "__main__":
    factor_return_and_special_return().run(param={"pool":"中证800","date":"20180424"})
